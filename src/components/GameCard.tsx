...
const GameCard: React.FC = () => {
	...
	const getRankIcon = (rank: number) => {
		switch (rank) {
			...
			default:
				return (
					<span className='w-4 h-4 flex items-center justify-center text-xs font-bold text-gray-500'>
						#{rank}
					</span>
				);
		}
		...
		<Card className='bg-white/70 dark:bg-gray-800/70 backdrop-blur-sm border-0 shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-105'>
			<CardHeader>
				<CardTitle className='flex items-center space-x-2'>
					...
					<span className='dark:text-white'>Space Shooter Game</span>
				</CardTitle>
				<CardDescription className='dark:text-white'>
					Take a break with our space shooter!
				</CardDescription>
			</CardHeader>
