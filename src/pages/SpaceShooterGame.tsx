...
const SpaceShooterGame = () => {
  ...
  const [player, setPlayer] = useState<GameObject>({ x: 375, y: 520, width: 50, height: 50 });
  ...
  useEffect(() => {
    ...
    const gameLoop = setInterval(() => {
      ...
      setBullets(prevBullets => {
        ...
        return prevBullets.filter((_, index) => 
          !prevBullets.some((bullet, bulletIndex) => 
            bulletIndex !== index && 
            enemies.some(enemy => 
              bullet.x < enemy.x + enemy.width &&
              bullet.x + bullet.width > enemy.x &&
  ...
  const renderGame = () => (
    <div className="relative mx-auto bg-black" style={{ width: gameWidth, height: gameHeight }}>
      {/* Stars background */}
        ...
        {gameState === 'menu' && (
          <div className="flex flex-col items-center justify-center min-h-[400px] space-y-8">
            <Card className="bg-white/10 backdrop-blur-md border-white/20 text-white">
              <CardHeader>
                <CardTitle className="text-center text-2xl">Space Shooter Game</CardTitle>
              </CardHeader>
